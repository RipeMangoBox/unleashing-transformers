...
class AttnBlock(nn.Module):
    def __init__(self, d_model, num_heads, activation, norm, drop_prob, tisa_num_kernels, seqlen, use_preln=False, d_ff=2048, bias=False, dilation=1):
        ...
        self.vallina_transformer = True
        if not self.vallina_transformer:
            ...
            self.attn = nn.MultiheadAttention(d_model, num_heads, dropout=drop_prob, batch_first=True)
    ...
    def forward(self, x):
        if self.use_preln:
            if not self.vallina_transformer:
                ...
                x = self.dropout_1(self.attn(x_norm, x_norm, x_norm, need_weights=False)[0]) + x
        else:
            if not self.vallina_transformer:
                x = self.dropout_1(self.attn(x)) + x
            else:
                x = self.dropout_1(self.attn(x, x, x, need_weights=False)[0]) + x
            x = self.norm_2(x)
