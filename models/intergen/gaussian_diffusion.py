...
from utils import *
from .intergen_utils.plot_script import *
from .intergen_utils.utils import *

from models.intergen.losses import InterLoss, GeometricLoss
...
class MotionDiffusion(GaussianDiffusion):
    ...
    def training_losses(self, model, mask, t_bar, *args, **kwargs):
        ...
        assert not torch.isnan(prediction).any(), "motion_pred contain NaN values"
        assert not torch.isinf(prediction).any(), "motion_pred contain infinite values"
        
        losses = {}
        losses["total"] = torch.mean(items['mse'])
