...
from models.intergen.common.quaternion import *
...
class MotionNormalizer():
    def __init__(self):
        # mean = np.load("./latents/global_mean.npy")
        # std = np.load("./latents/global_std.npy")
        mean = torch.load("./latents/global_mean.pt")
        std = torch.load("./latents/global_std.pt")

        self.motion_mean = mean.cuda(0)
        self.motion_std = std.cuda(0)
...
def print_current_loss(start_time, niter_state, losses, epoch=None, inner_iter=None, lr=None):
    ...
    if epoch is not None and lr is not None :
        print('epoch: %3d niter:%6d inner_iter:%4d lr:%9fe-4' % (epoch, niter_state, inner_iter, lr*10000), end=" ")
